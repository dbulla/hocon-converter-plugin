...
import com.nurflugel.hocon.generators.ConfGenerator.outputTopLevelListsAtBottom
...
object PropertiesGenerator {
   ...
   */
  fun generatePropertiesOutput(
    propertiesMap: PropertiesMap,
    project: Project
  ): MutableList<String> {

    val putTopLevelListsAtBottom = ProjectSettings.Companion.putTopLevelListsAtBottom(project)
    ...
    populateOutputLinesFromMap(propertiesMap.map, outputLines, keyStack, putTopLevelListsAtBottom)
    if (putTopLevelListsAtBottom)
      outputTopLevelListsAtBottom(outputLines, propertiesMap.map)
  ...
  private fun populateOutputLinesFromMap(
    map: HoconMap,
    ...
    for (key in map.getKeys().toSortedSet()) {
      ...
      when (value) {
        is HoconMap -> {
          ...
          keyStack.push(key)
          populateOutputLinesFromMap(value, outputLines, keyStack, putTopLevelListsAtBottom)
        }
        else -> {
          ...
          when (value) {
            is HoconList -> {
              ...
              if (!putTopLevelListsAtBottom || indentLevel != 0) {
  ...
  private fun populateOutputLinesFromList(
    list: HoconList,
    outputLines: MutableList<String>,
    fullKey: String,
    indentLevel: Int
